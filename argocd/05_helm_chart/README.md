...
- 매칭 Git 경로: `github/05_helm_chart`
...
# 2. argocd/05_helm_chart/argo_setup.yaml 설명
...
```bash
awk -v repo="$REPO_URL" '{gsub(/\$\{REPO_URL\}/,repo)}1' argocd/05_helm_chart/argo_setup.yaml \
...
- `github/05_helm_chart/values.yaml`의 `image.tag` 변경 후 push
